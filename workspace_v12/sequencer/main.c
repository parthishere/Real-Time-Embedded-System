...
volatile uint32_t lastInterruptTime = 0; // Global variable to store the last interrupt time
volatile bool taskSignalFlag = false;    // Flag to signal the "task" for processing
// Placeholder function for task signaling or processing
void signalProcessingTask(uint32_t timestamp);
...
void Timer0IntHandler(void)
{
    ...
    // Capture the current timer value (timestamp)
    uint32_t timestamp = TimerValueGet(TIMER0_BASE, TIMER_A);

    // Signal the processing task or handle the timing directly
    signalProcessingTask(timestamp);
    taskSignalFlag = true;
    // Store the timestamp for checking the period
...
void setupTimer(void)
{
    ...
    // Load value to the timer for 1-second intervals at 80MHz system cloc

    uint32_t timerLoadValue = 80000000 - 1;
...
void setupInterrupts(void)
{
    ...
    // Enable the interrupt in the NVIC with a specified priority
...
int main(void)
{
    // Set system clock to 80MHz using the PLL and an external 16MHz crystal
    ...
    while (1)
    {
        // Main loop checks if the task signal flag is set
        if (taskSignalFlag)
        {
            // If set, process the task with the last interrupt timestamp
            signalProcessingTask(lastInterruptTime);
        }
    }
}
void signalProcessingTask(uint32_t timestamp)
{
    // Simulated task processing, prints the timestamp indicating task execution
    UARTprintf("Processing Task at the %u time.\n", timestamp);
    // Reset the task signal flag after processing
    taskSignalFlag = false;
}
...
//#define ITERATION 11800
//#define MULTIPLIER 1
//#define Hz (30 * MULTIPLIER)        // Hz
//#define SEQUENCER_COUNT 900
...
//            execution_cycle[0]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 1 (Frame Sampler thread) Start Time:%d ms, Release count %d \n\r", xCurrentTick, execution_cycle[0]);
//            fibonacci();
...
//
//            UARTprintf("Task 1 (Frame Sampler thread) Completion Time:%d ms, Execution Time:%d ms\n\r", xFibTime, total_time);
//        }
//    }
//    vTaskDelete( NULL );
//}
...
//            execution_cycle[1]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 2 (Time-stamp with Image Analysis thread) Start Time:%d ms, Release count %d \n\r", xCurrentTick, execution_cycle[1]);
//            fibonacci();
//            TickType_t xFibTime = xTaskGetTickCount();
//            TickType_t total_time = (xFibTime - xCurrentTick);
//            execution_time[1] += total_time;
//            if(wcet[1] < total_time) wcet[1] = total_time;
//            UARTprintf("Task 2 (Time-stamp with Image Analysis thread) Completion Time:%d ms, Execution Time:%d ms\n\r", xFibTime, (xFibTime - xCurrentTick));
//        }
//    }
//    vTaskSuspend( NULL );
//}
...
//            execution_cycle[2]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 3 (Difference Image Proc thread) Start Time:%d ms, Release count %d \n\r", xCurrentTick, execution_cycle[2]);
//            fibonacci();
//            TickType_t xFibTime = xTaskGetTickCount();
//            TickType_t total_time = (xFibTime - xCurrentTick);
//            execution_time[2] += total_time;
//            if(wcet[2] < total_time) wcet[2] = total_time;
//            UARTprintf("Task 3 (Difference Image Proc thread) Completion Time:%d ms, Execution Time:%d ms\n\r", xFibTime, (xFibTime - xCurrentTick));
//        }
//    }
//    vTaskDelete( NULL );
//}
...
//            execution_cycle[3]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 4 (Time-stamp Image Save to File thread) Start Time:%d ms, Release count %d \n\r", xCurrentTick, execution_cycle[3]);
//            fibonacci();
//            TickType_t xFibTime = xTaskGetTickCount();
//            TickType_t total_time = (xFibTime - xCurrentTick);
//            execution_time[3] += total_time;
//            if(wcet[3] < total_time) wcet[3] = total_time;
//            UARTprintf("Task 4 (Time-stamp Image Save to File thread) Completion Time:%d ms, Execution Timet:%d ms\n\r", xFibTime, (xFibTime - xCurrentTick));
//        }
//    }
//    vTaskDelete( NULL );
//}
...
//            execution_cycle[4]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 5 (Processed Image Save to File thread) Start Time:%d ms, Release count %d \n\r", xCurrentTick, execution_cycle[4]);
//            fibonacci();
//            TickType_t xFibTime = xTaskGetTickCount();
//            TickType_t total_time = (xFibTime - xCurrentTick);
//            execution_time[4] += total_time;
//            if(wcet[4] < total_time) wcet[4] = total_time;
//            UARTprintf("Task 5 (Processed Image Save to File thread) Completion Time:%d ms, Execution Time:%d ms\n\r", xFibTime, (xFibTime - xCurrentTick));
//        }
//    }
//    vTaskDelete( NULL );
//}
...
//            execution_cycle[5]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 6 (Send Time-stamped Image to Remote thread) Start Time:%d ms, Release count %d \n\r", xCurrentTick, execution_cycle[5]);
//            fibonacci();
//            TickType_t xFibTime = xTaskGetTickCount();
//            TickType_t total_time = (xFibTime - xCurrentTick);
//            execution_time[5] += total_time;
//            if(wcet[5] < total_time) wcet[5] = total_time;
//            UARTprintf("Task 6 (Send Time-stamped Image to Remote thread) Completion Time:%d ms, Execution Time:%d ms\n\r", xFibTime, (xFibTime - xCurrentTick));
//        }
//    }
//    vTaskDelete( NULL );
//}
...
//            execution_cycle[6]++;
//            TickType_t xCurrentTick = xTaskGetTickCount();
//            UARTprintf("Task 7 (10 sec Tick Debug thread) Start Time:%d ms , Release count %d \n\r", xCurrentTick, execution_cycle[6]);
//            fibonacci();
//            TickType_t xFibTime = xTaskGetTickCount();
//            TickType_t total_time = (xFibTime - xCurrentTick);
//            execution_time[6] += total_time;
//            if(wcet[6] < total_time) wcet[6] = total_time;
//            UARTprintf("Task 7 (10 sec Tick Debug thread) Completion Time:%d ms, Execution Time:%d ms\n\r", xFibTime, (xFibTime - xCurrentTick));
//        }
//    }
//    vTaskDelete( NULL );
//}
