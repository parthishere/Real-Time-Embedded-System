...
/*********************Random back off scheme to avoid deadlock********************/
int random_backoff_scheme(void)
{
  int random_backoff_time;
  random_backoff_time = (rand() % 3) + 2;   // Generating delay between 2 to 5 seconds (2 added as minimum delay needed is 2 to avoid deadlock) 
...
void *grabRsrcs(void *threadp)
{
   ...
   {
     //Random backoff delay for thread 2 so that thread 1 can acquire the mutex rsrcB and finish execution
     if(backoff)
     { 
