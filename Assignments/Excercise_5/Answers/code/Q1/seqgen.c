...
#define HUNDRED_MS_ITERATION_COUNT 466500 // 100 ms load
#define LOAD_MS (100 / MULTIPLIER) // ms load
#define SEQUENCER_DELAY_NS (33333333 / MULTIPLIER)
...
static int calibrated_load_value = 0;
...
void main(void)
{
    ...
    printf("Starting Sequencer Demo @ 30Hz with multiplier %d \n", MULTIPLIER);
    syslog(LOG_CRIT, "Starting Sequencer Demo\n");

    double avg_time = 0;


    calibrated_load_value = 0;
    read_time(&start);
    while(end - start <= LOAD_MS){
        fibTest(1000);
        read_time(&end);
        calibrated_load_value++;
    }
    calibrated_load_value *= 1000;

    printf("**** Calibrated Load value for %d ms load is %d ****\n\r",LOAD_MS, calibrated_load_value);
    printf("testing Fib load with iterations :%d\n", calibrated_load_value);
    
    for(int i=0;i<10;i++){
        
        read_time(&start);
        fibTest(calibrated_load_value);
        read_time(&end);
    ...
    threadParams[0].sequencePeriods = 900 * MULTIPLIER;
...
{
    struct timeval current_time_val;
    struct timespec delay_time = {0, SEQUENCER_DELAY_NS}; // delay for 33.33 msec, 30 Hz
