...
int deadline_monotonic_feasibility(U32_T numServices, U32_T period[],
...
int main(void)
{
    ...
    printf("Ex-5 U=%4.2f\% (C1=1, C2=1, C3=1; T1=2, T2=5, T3=10; T=D): ",
           ((1.0 / 2.0) * 100.0 + (1.0 / 5.0) * 100.0 + (1.0 / 10.0) * 100.0));
    numServices = 3;
    printf("\nCompletion Test Feasibility: ");
    ...
    if (edf_feasibility(numServices, ex5_period, ex5_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");

    printf("LLF Feasibility: ");
    if (llf_feasibility(numServices, ex5_period, ex5_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");
    ...
    printf("\nDeadline Monotonic Test Feasibility: ");
    if (deadline_monotonic_feasibility(numServices, ex6_period, ex6_wcet, ex6_deadline) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");

    printf("EDF Feasibility on deadline: ");
    if (edf_feasibility(numServices, ex6_deadline, ex6_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");
    ...
    printf("EDF Feasibility: ");
    if (edf_feasibility(numServices, ex7_period, ex7_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");

    printf("LLF Feasibility: ");
    if (llf_feasibility(numServices, ex7_period, ex7_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");
    ...
    printf("EDF Feasibility: ");
    if (edf_feasibility(numServices, ex8_period, ex8_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");

    printf("LLF Feasibility: ");
    if (llf_feasibility(numServices, ex8_period, ex8_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");
    ...
    printf("EDF Feasibility: ");
    if (edf_feasibility(numServices, ex9_period, ex9_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");

    printf("LLF Feasibility: ");
    if (llf_feasibility(numServices, ex9_period, ex9_wcet) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");
...
int edf_feasibility(U32_T numServices, U32_T period[], U32_T wcet[])
{
    ...
    {
        U_total += (double)wcet[i] / period[i];
    }
    printf("\nUtility Sum for EDF is: %f\n", U_total);
    if (U_total <= 1.0)
        return TRUE;
    else
        return FALSE;
                                   ...
                                   U32_T wcet[], U32_T deadline[])
{
    ...
    for (int i = 0; i < numServices; i++)
    {
        ...
        for (int j = 0; j <= i; j++)
        {
            Interference += (ceil(deadline[i] / period[j])) * wcet[j];
        }
        utilization = ((float)(wcet[i]) / (float)deadline[i]) + (Interference / (float)deadline[i]);
        if (utilization > 1)
        {
            status = 1;
            break;
